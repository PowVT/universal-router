...
import type { AdvancedOrder, ConsiderationItem, CriteriaResolver, OfferItem, OrderComponents } from "../types";

const deployConstants = require("../constants");
...
export const marketplaceFixture = async (
  ...
  const directMarketplaceContract = await deployContract<ConsiderationInterface>(
  ...
  const marketplaceContractAddress = await create2Factory.findCreate2Address(
    ...
    marketplaceContractFactory.bytecode + conduitController.address.slice(2).padStart(64, "0"),
  );
  ...
  const createMirrorBuyNowOrder = async (
    ...
    for (const { itemType, token, identifierOrCriteria, startAmount, endAmount } of order.parameters.offer) {
      if (
        ...
        compressedOfferItems.push({
          ...
          startAmount: startAmount.eq(endAmount) ? startAmount : startAmount.sub(1),
        ...
        compressedOfferItems[index].startAmount = compressedOfferItems[index].startAmount.add(
          startAmount.eq(endAmount) ? startAmount : startAmount.sub(1),
        );
        compressedOfferItems[index].endAmount = compressedOfferItems[index].endAmount.add(
    ...
    for (const { itemType, token, identifierOrCriteria, startAmount, endAmount, recipient } of order.parameters
      .consideration) {
      if (
        ...
        compressedConsiderationItems.push({
          ...
          token,
          identifierOrCriteria,
          startAmount: startAmount.eq(endAmount) ? startAmount : startAmount.add(1),
        ...
        compressedConsiderationItems[index].startAmount = compressedConsiderationItems[index].startAmount.add(
          startAmount.eq(endAmount) ? startAmount : startAmount.add(1),
        );
        compressedConsiderationItems[index].endAmount = compressedConsiderationItems[index].endAmount.add(
