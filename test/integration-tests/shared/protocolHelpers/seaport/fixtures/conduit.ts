...
export const conduitFixture = async (create2Factory: any, owner: Wallet) => {
  let conduitController: ConduitControllerInterface;
  let conduitImplementation: Conduit__factory;
  if (process.env.REFERENCE) {
    conduitImplementation = (await ethers.getContractFactory("ReferenceConduit")) as Conduit__factory;
    conduitController = await deployContract("ConduitController", owner);
  } else {
    conduitImplementation = await ethers.getContractFactory("Conduit") as Conduit__factory;
    ...
    conduitController = (await ethers.getContractAt("ConduitController", conduitControllerAddress, owner)) as any;
  }
  ...
  const deployNewConduit = async (owner: Wallet, conduitKey?: string) => {
    ...
    if (!process.env.REFERENCE) {
      ...
      await expect(conduitController.connect(owner).createConduit(assignedConduitKey, constants.AddressZero)).to.be
        .reverted;
    }
