...
import type { Reenterer } from "../../../../../../typechain";
...
export const seaportFixture = async (owner: Wallet) => {
  ...
  console.log("Hi from fixture")
  const { conduitController, conduitImplementation, conduitKeyOne, conduitOne, getTransferSender, deployNewConduit } =
  ...
  } = await marketplaceFixture(create2Factory, conduitController, conduitOne, chainId, owner);
  ...
  const withBalanceChecks = async (
    ...
    for (const offeredItem of allOfferedItems as any[]) {
      ...
      if (offeredItem.itemType === 0) {
        ...
        // ERC1155
        offeredItem.finalBalance = await tokenByType[offeredItem.itemType].balanceOf(
          offeredItem.account,
          offeredItem.identifierOrCriteria,
        );
      } else if (offeredItem.itemType < 3) {
  ...
  const checkExpectedEvents = async (
    ...
    if (criteriaResolvers && criteriaResolvers.length) {
      for (const { orderIndex, side, index, identifier } of criteriaResolvers) {
        ...
        orderGroups[orderIndex].order.parameters[side === 0 ? "offer" : "consideration"][index].identifierOrCriteria =
          identifier;
      }
    }
          ...
          const transferLogs = (tokenEvents ?? [])
            ...
            .filter(
              x =>
                ...
                (recipient !== constants.AddressZero ? x.args.to === recipient : true),
            );
